...
#h10_address = "EF:3A:AB:34:64:66" #Black Windows
...
loop.run_until_complete(device_scan())
#asyncio.run(device_scan())
...
async def run(client, debug=False):
    ...
    while(True):
        ...
        t = t+1
        #x = ecg_session_data[n : n+130]
        data_length = len(ecg_session_data)
        if(data_length>390):
            x = ecg_session_data[data_length-390:data_length]
            #n = n + 130
            plt.plot(x)    
            plt.show()
